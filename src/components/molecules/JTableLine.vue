<template lang="pug">
  div
    ...
    template(v-for="(column, index) in columns")
      el-input(v-if="column.typeValue === 'string'", v-model="task.data[column.value]", :style="{ width: column.width + 'px' }")
      el-date-picker(v-else-if="column.typeValue === 'date'", v-model="task.data[column.value]", format="M/d", :style="{ width: column.width + 'px' }")
      span(v-if="index === 0")
        .task-action-space(v-if="type === 'task'")
